...
<body>
	...
	<hr>
    <?php 
    // PHP'deki @ işareti, hata raporlama mekanizmasını geçici olarak devre dışı bırakmak için kullanılır.
	if (@$_GET['durum']=="ok") {
		echo "İşlem başarılı";
	} 
    ...
    */
    ...
    //Where Select İşlemi
    $bilgilerimsor=$db->prepare("SELECT * from bilgilerim where bilgilerim_yas=:yas");
	$bilgilerimsor->execute(array('yas' => 22));
	while($bilgilerimcek=$bilgilerimsor->fetch(PDO::FETCH_ASSOC)) {
		echo $bilgilerimcek['bilgilerim_ad']; echo "<br>";
	}
